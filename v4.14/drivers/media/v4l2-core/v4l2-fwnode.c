          ...
          struct v4l2_async_subdev *asd))
{
  ...
  if (WARN_ON(asd_struct_size < sizeof(struct v4l2_async_subdev)))
    return -EINVAL;
  dev_info(dev, "dev_fwnode:%p, is_available:%d", dev_fwnode(dev), dev_fwnode(dev)?fwnode_device_is_available(dev_fwnode(dev)) :0);
